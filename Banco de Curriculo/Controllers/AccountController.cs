...
using Banco_de_Curriculo.Models;

namespace Banco_de_Curriculo.Controllers
{
    [Authorize]
    public class AccountController : Controller
    {
        ...
        [AllowAnonymous]
        [ValidateAntiForgeryToken]
        public async Task<ActionResult> Register(RegisterViewModel model, HttpPostedFileBase foto, HttpPostedFileBase curriculo)
        {
            using (var db = new ApplicationDbContext())
            {
                //valida se o cpf já existe
                if(db.Users.Where(x=>x.Cpf == model.Cpf).FirstOrDefault() != null)
                {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (foto != null)
                {
                    fotoModel = foto.FileName + DateTime.Now.ToString();
                    foto.SaveAs(HttpContext.Server.MapPath("~/UsersFotos/") + fotoModel);

                }
                var curriculoModel = "";
                if (curriculo != null)
                {
                    curriculoModel = curriculo.FileName + DateTime.Now.ToString();
                    curriculo.SaveAs(HttpContext.Server.MapPath("~/UsersCurriculos/") + curriculoModel);
                ...
                if (result.Succeeded)
                {
                    ...
                    // string code = awaApplicationDbContextt UserManager.GenerateEmailConfirmationTokenAsync(user.Id);
